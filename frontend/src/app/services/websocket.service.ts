...
export class WebsocketService {
  ...
  connect(username: string, id: string) {
    ...
    this.stompClient.onConnect = (frame) => {
      ...
      this.stompClient?.subscribe('/topic/' + id, (message: Message) => {
      ...
      this.stompClient?.publish({
        destination: '/app/chat.addUser/' + id,  // Server endpoint for adding users
  ...
  sendMessage(username: string, content: string, id: string) {
    if (this.stompClient && this.stompClient.connected) {
      ...
      // Publish (send) the message to the '/app/chat.sendMessage/${id}' destination
      this.stompClient.publish({
