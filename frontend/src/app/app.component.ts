...
export class AppComponent implements OnInit {
  ...
  id: string = ''; // Stores the id entered by the user
  ...
  connect() {
    ...
    this.websocketService.connect(this.username, this.id);  // Call the WebSocket service to connect
  }

  sendMessage() {
    if (this.message) {
      this.websocketService.sendMessage(this.username, this.message, this.id);  // Send the message via WebSocket service
